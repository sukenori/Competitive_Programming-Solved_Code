...
//     HH、MM、SSはそれぞれ時間、分、秒を2桁で表した文字列
    string HH, MM, SS;
    HH = to_string (hour); if (HH.size()==1) HH = "0" + HH;
    MM = to_string (minute); if (MM.size()==1) MM = "0" + MM;
    SS = to_string (second); if (SS.size()==1) SS = "0" + SS;
    return HH + ":" + MM + ":" +SS;
  }
...
//     diff_second の値は -86400 ~ 86400 の範囲を取とりうる
    int nts;
    nts= (hour * 60 + minute) * 60 + second + diff_second;
    if (nts >= 86400) nts -= 86400;
    if (nts < 0) nts += 86400;
    second = nts % 60;
    minute = (nts / 60) % 60;
    hour = nts / 3600;
